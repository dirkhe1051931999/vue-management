<template>
  <div class="laboratory right-container">
    <Menu />
    <Header />
    <!-- 标题 -->
    <div class="laboratory-header">
      <h2><span>项目列表</span></h2>
      <button
    ...
    <!-- 项目列表：Project -->
    <Project
      v-for="(project, index) in itemlist"
      :project="project"
      :position="`${index % 2 === 0 ? 'left' : 'right'}`"
      :key="project.id"
      :currentIndex="index"
      @deleteLab="deleteLab"
      @confirmLaboratory="confirmLaboratory"
    />
    <!-- 添加项目：Dialog -->
    <Dialog
      :title="dialogTitle"
  ...
  methods: {
    ...
    async getLaboratories() {
      ...
      if (res.success === 1) {
        ...
        this.setItemList(res.laboratories)
      }
    },
    hideDialog() {
    ...
    confirmLaboratory(data) {
      this.updateMyItem(data)
    }
  },
  created() {
    this.laboratory = Object.assign({}, this.newLaboratory);
